...
def make_env_and_datasets(env_name, frame_stack=None, action_clip_eps=1e-5):
    ...
    elif 'GMM' in env_name or 'GMM-2D' in env_name:
        # todo: A toy env for sexy visualzation
        from envs import MulitCrescentEnv
        import matplotlib.pyplot as plt
        env = MulitCrescentEnv.MulitCrescentEnv()
        eval_env = MulitCrescentEnv.MulitCrescentEnv()
        dataset = MulitCrescentEnv.generate_offline_dataset(env, num_points_per_quadrant=500, skip=['top_right', 'bottom_left'])
        train_dataset, val_dataset = dataset, None
        
        # import pdb; pdb.set_trace()
        # plt.close()
        # ax = env.vizualize()
        # # plt.scatter(dataset['actions'][:,0], dataset['actions'][:,1], color='blue', marker='x', s=50, alpha=0.5)
        # # plt.scatter(action_array[:, 0], action_array[:, 1], marker='*', s=300, color='yellow', alpha=1, edgecolor='black', label='Actions')
        # plt.tight_layout() 
        # plt.savefig('viz/ToyEnvViz.pdf', dpi=300, bbox_inches='tight', pad_inches=0.01)
