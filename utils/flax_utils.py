...
class TrainState(flax.struct.PyTreeNode):
    ...
    def apply_loss_fn(self, loss_fn):
        ...
        # # [zhanghe] clip the grad
        # def clip_gradients(grads, max_norm):
        #     # 获取所有叶子节点（梯度数组），展平后拼接成一个长向量
        #     grads_flat = jax.tree_util.tree_leaves(grads)
        #     grads_concat = jnp.concatenate([jnp.ravel(g) for g in grads_flat])

        #     # 计算 L2 范数
        #     total_norm = jnp.linalg.norm(grads_concat, ord=2)
        ...
        # clipped_grad = clip_gradients(grads, max_norm=2.0)
        
        # grad_max = jax.tree_util.tree_map(jnp.max, clipped_grad)
        # grad_min = jax.tree_util.tree_map(jnp.min, clipped_grad)
        # grad_norm = jax.tree_util.tree_map(jnp.linalg.norm, clipped_grad)
        ...
        # final_grad_min = jnp.min(grad_min_flat)
        # final_grad_norm = jnp.linalg.norm(grad_norm_flat, ord=1)
        # final_grad_norm2 = jnp.linalg.norm(grad_norm_flat, ord=2)
