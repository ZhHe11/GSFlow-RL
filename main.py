...
flags.DEFINE_string('exp_name', 'gsflow', 'Exp name')
flags.DEFINE_bool('is_wandb', True, 'use wandb or debug')
flags.DEFINE_integer('monte_carlo', 0, 'Whether to use Monte Carlo evaluation.')
flags.DEFINE_integer('only_eval', 0, 'only_eval')

config_flags.DEFINE_config_file('agent', 'agents/gsflow.py', lock_config=False)

flags.DEFINE_float('offline_temperature', 0, 'to control the exploration')
flags.DEFINE_float('online_temperature', 1, 'to control the exploration')
flags.DEFINE_float('offline_alpha', 50, 'to control the exploration')
flags.DEFINE_float('online_alpha', 50, 'to control the exploration')


def main(_):
    # Set up logger.
    exp_name = FLAGS.exp_name + '-' + get_exp_name(FLAGS.seed)
    project_name = 'gsflow-rl'
    FLAGS.save_dir = os.path.join(FLAGS.save_dir, project_name, FLAGS.run_group, exp_name)

    wandb_path = os.path.join('./', FLAGS.save_dir)
    os.makedirs(wandb_path, exist_ok=True)
    if FLAGS.is_wandb:
        setup_wandb(project=project_name, group=FLAGS.run_group, name=exp_name, output_dir=wandb_path)
    ...
    if FLAGS.monte_carlo or FLAGS.only_eval:
        only_evaluation = 1
    ...
    for i in tqdm.tqdm(range(1, FLAGS.offline_steps + FLAGS.online_steps + 1), smoothing=0.1, dynamic_ncols=True):      
        if only_evaluation:
            ...
            elif config['agent_name'] == 'gsflow':
                agent, update_info = agent.update(batch, temperature=FLAGS.offline_temperature, alpha=FLAGS.offline_alpha)
            else:
                agent, update_info = agent.update(batch)
            end_time = time.time()
            time_list.append(1000 * (end_time - start_time))

        else:
            # Online fine-tuning.
            if online_flag:
                print('Online fine-tuning...')
                online_flag = 0
            ...
            if config['agent_name'] == 'rebrac':
                ...
                agent, update_info = agent.update(batch, temperature=FLAGS.online_temperature, alpha=FLAGS.online_alpha)
            else:
                agent, update_info = agent.update(batch)
        ...
        # Evaluate agent.
        if FLAGS.eval_interval != 0 and (i % FLAGS.eval_interval == 0) or FLAGS.only_eval == 1 or i == 1:
            renders = []
            eval_metrics = {}
            if i == 1 and FLAGS.only_eval != 1:
                num_eval_episodes = 1
            ...
            eval_info, trajs, cur_renders = evaluate(
                ...
                monte_carlo=FLAGS.monte_carlo,
                viz_GMM=True if 'GMM' in FLAGS.env_name else False,
